...
class _TambahLapanganState extends State<TambahLapangan> {
  ...
  // Dummy data untuk dropdown (nanti dari database)
  final List<String> _tipeLapanganOptions = [
    'Indoor',
    'Outdoor',
    'Semi Indoor',
  ];

  final List<String> _jenisOlahragaOptions = [
    'Futsal',
    'Badminton',
    'Basket',
    'Voli',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            // Custom Header - Fixed height, tidak menggunakan Positioned
            Container(
              height: 200,
            ...
            // Form Container - Sekarang dalam SingleChildScrollView
            Transform.translate(
              offset: const Offset(0, -50), // Overlap dengan header
              child: Container(
            ...
            child: IconButton(
              ...
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Map picker akan diimplementasi nanti'),
                  ),
                );
              },
            ),
        ...
        Container(
          ...
          child: InkWell(
            onTap: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Image picker akan diimplementasi nanti'),
                ),
              );
            },
            borderRadius: BorderRadius.circular(16),
                        ...
                        onTap: () {
                          if (laneFormKey.currentState!.validate() &&
                            ...
                            setState(() {
                              _lanes.add(
                                LaneData(
                                  namaLane: namaLaneController.text,
                                  jenisOlahraga: selectedJenisOlahragaLane!,
                                  deskripsi: deskripsiLaneController.text,
                                  kapasitas: int.parse(
                                    kapasitasLaneController.text,
                                  ),
                                  hargaPerJam: int.parse(
                                    hargaLaneController.text,
                                  ),
                                  status: statusLane,
                                ),
                              );
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: const Text('Lane berhasil dihapus'),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
            'Apakah Anda yakin ingin menyimpan data lapangan ini? Pastikan semua informasi sudah benar.',
          ),
          actions: [
            TextButton(
              onPressed: () {
  ...
  void _submitForm() {
    ...
    Future.delayed(const Duration(seconds: 2), () {
      ...
      final formData = {
        ...
        'tipe_lapangan': _selectedTipeLapangan,
        'jenis_olahraga': _selectedJenisOlahraga,
        'harga_per_jam': int.tryParse(_hargaController.text),
        'kapasitas': int.tryParse(_kapasitasController.text),
        'alamat': _alamatController.text,
        'kecamatan': _kecamatanController.text,
        'kabupaten': _kabupatenController.text,
        'provinsi': _provinsiController.text,
        'koordinat': _koordinatController.text,
        'fasilitas': _selectedFasilitas,
        'status': _statusLapangan,
        'lanes': _lanes.map((lane) => lane.toMap()).toList(),
      };
        ...
        SnackBar(
          content: const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Data lapangan berhasil disimpan!'),
            ],
          ),
          backgroundColor: Colors.green,
          behavior: SnackBarBehavior.floating,
